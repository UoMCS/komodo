...
typedef enum
{ NONTERM, TERM, TERM2FREE, NONTERM2FREE }
RuleElementType;
// Two separate flags:  TERM points at a string, NONTERM doesn't ("TERMinal node")
// 2FREE indicates a `non-permanent' record

typedef enum
{ TRANRULE, INTRULE, RELATIVERULE, UINTRULE, URELATIVERULE }
RuleType;
...
int asm_assemble_wrapper(char *, GList *, char *, unsigned int, int,
                         char *, int);
