...
struct memory_segment_name
{                                                          /* Memory Segments */
  unsigned char *start;/*chararr ptr to addr of length board->memory_ptr_width */
  unsigned char *length;          /* chararr of length board->memory_ptr_width */
};

struct reg_bank_name
{                                                           /* Regbank struct */
...
typedef struct feature_name        feature;
typedef struct memory_segment_name memory_segment;
typedef struct reg_bank_name       reg_bank;
typedef struct special_reg_name    special_reg;

struct target_system_name
{                                 /* Record defining back-end characteristics */
...
typedef struct target_system_name target_system;
...
special_reg special_registers[SPECIAL_REGISTER_COUNT];	// #!#!# Should go in `board' record too ???@@@
...
char download_send_packet(int, unsigned int, uchar*);
